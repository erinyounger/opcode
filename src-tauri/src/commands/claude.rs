                ...
                if let Some(message) = entry.message {
                    if message.role.as_deref() == Some("user") {
                        if let Some(content) = message.content {
                            ...
                            // Found a valid user message - return it
...
/// Extracts the last message timestamp from a JSONL file (for sorting by last activity)
fn extract_last_message_timestamp(jsonl_path: &PathBuf) -> Option<String> {
    ...
    // Read all lines and keep track of the last valid timestamp
    for line in reader.lines() {
        if let Ok(line) = line {
            if let Ok(entry) = serde_json::from_str::<JsonlEntry>(&line) {
...
/// Checks if a JSONL file is empty or has no valid content
fn is_jsonl_file_empty(jsonl_path: &PathBuf) -> bool {
    ...
    // Try to read and check if there are any valid JSON lines
    let file = match fs::File::open(jsonl_path) {
        Ok(file) => file,
        Err(_) => return true, // If we can't open it, consider it empty
    };
    ...
    for line in reader.lines() {
        if let Ok(line) = line {
            ...
            if serde_json::from_str::<serde_json::Value>(trimmed).is_ok() {
                ...
                break; // Found at least one valid JSON line
            }
        }
    }
